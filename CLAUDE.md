...
2. **`MealLogAnalysisService`**
   ...
   - Handles markdown-wrapped JSON responses (strips ```json blocks before parsing)
...
- **Two separate login paths:**
  - Nutritionists: `/nutritionists/sign_in`
  - Patients: `/patients/sign_in`
...
- Main patient dashboard at `/pats/dashboard`
...
- Root path: `pages#home` (shows login links for both user types)
...
1. Initialize with `@chat = RubyLLM.chat` (or `@chat = RubyLLM.chat(model: 'gpt-4o')` for vision)
...
- **Dual authentication**: The home page shows separate login links for nutritionists and patients
